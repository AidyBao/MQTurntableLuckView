...
-(void)initUI{
    // 转盘
    self.rotateWheel = [[UIImageView alloc]initWithFrame:self.frame];
    ...
    self.playButton = [[UIButton alloc]initWithFrame:
                       CGRectMake(0,0,CGRectGetWidth(self.frame)/3,CGRectGetHeight(self.bounds)/3)];
    self.playButton.center = CGPointMake(CGRectGetWidth(self.frame)/2, CGRectGetWidth(self.frame)/2);
    self.playButton.layer.cornerRadius = CGRectGetWidth(self.frame)/3/2;
    [self addSubview:self.playButton];
    
    // 外围装饰背景图
    UIImageView * backImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 330*turnScale_W, 345*turnScale_H)];
    backImageView.image = [UIImage imageNamed:@"2_0000_up"];
    [self addSubview:backImageView];
    //    backImageView.centerX = self.rotateWheel.center.x;
    
    
    [backImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self.rotateWheel.mas_centerX).with.offset(-1*turnScale_W);        make.centerY.mas_equalTo(self.rotateWheel.mas_centerY).with.offset(-13*turnScale_H);
    ...
    for (int i = 0; i < 8; i ++) {
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0,M_PI * CGRectGetHeight(self.rotateWheel.frame)/8,CGRectGetHeight(self.rotateWheel.frame)/2)];
        label.layer.anchorPoint = CGPointMake(0.5, 1);
        label.center = CGPointMake(CGRectGetHeight(self.rotateWheel.frame)/2, CGRectGetHeight(self.rotateWheel.frame)/2);
